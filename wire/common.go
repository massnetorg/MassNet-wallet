...
// Int64 reads eight bytes from the provided reader using a buffer from the
// free list, converts it to a number using the provided byte order, and returns
// the resulting int64.
func (l binaryFreeList) Int64(r io.Reader, byteOrder binary.ByteOrder) (int64, error) {
	buf := l.Borrow()[:8]
	if _, err := io.ReadFull(r, buf); err != nil {
		l.Return(buf)
		return 0, err
...
// PutUint8 copies the provided uint8 into ZXa buffer from the free list and
