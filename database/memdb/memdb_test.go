...
import (
	...
	"github.com/massnetorg/MassNet-wallet/database/memdb"
...
func TestClosed(t *testing.T) {
	...
	_, err = db.InsertBlock(massutil.NewBlock(config.ChainParams.GenesisBlock))
	if err != nil {
...
func TestMemDb(t *testing.T) {
	...
	db.UpdateAddrIndexForBlock(&wire.Hash{}, 0, database.BlockAddrIndex{})
	db.DeleteAddrIndex()
	db.InsertRootKey([]byte(""), "", true)
	db.FetchRootKey("")
	db.FetchAllRootPkStr()
	db.InitChildKeyNum("")
	db.FetchChildKeyNum("")
	db.UpdateChildKeyNum("")
	db.InsertWitnessAddr("", []byte(""))
	db.FetchWitnessAddrToRedeem()
	db.ImportChildKeyNum("", "")
	...
	genesisHash := config.ChainParams.GenesisHash
	if err := db.DropAfterBlockBySha(genesisHash); err != nil {
		t.Errorf("DropAfterBlockBySha: unexpected error %v", err)
	}
	...
	if _, err := db.FetchHeightRange(0, 1); err != nil {
		t.Errorf("FetchHeightRange: unexpected error %v", err)
	}
