...
import (
	...
	"massnet.org/mass-wallet/poc/pocutil/crypto"
	"massnet.org/mass-wallet/poc/pocutil/crypto/hash"
)
...
func (d *digest) UnmarshalBinary(b []byte) error {
	if len(b) < len(magic224) || (d.is224 && string(b[:len(magic224)]) != magic224) || (!d.is224 && string(b[:len(magic256)]) != magic256) {
		return errors.New("massnet.org/mass-wallet/poc/pocutil/crypto/sha256: invalid hash state identifier")
	}
	if len(b) != marshaledSize {
		return errors.New("massnet.org/mass-wallet/poc/pocutil/crypto/sha256: invalid hash state size")
	}
	b = b[len(magic224):]
