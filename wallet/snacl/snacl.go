...
func NewSecretKeyFromUnmarshal(password []byte, salt []byte, N, r, p int) (*SecretKey, error) {
	sk := SecretKey{
		Key: (*CryptoKey)(&[KeySize]byte{}),
	}
	// setup parameters
	...
	err := sk.deriveKeyFromUnmarshal(password, salt)
	if err != nil {
		return nil, err
	}
...
//
func (sk *SecretKey) deriveKeyFromUnmarshal(password []byte, salt []byte) error {
