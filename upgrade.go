...
func oldMassHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "mass")
	}
...
// location prior to mass version 0.2.0 and uses heuristics to ascertain the old
...
func upgradeDBPathNet(oldDbPath, netName string) error {
	...
	if err == nil {
		...
		newDbRoot := filepath.Join(filepath.Dir(cfg.Db.DataDir), netName)
...
// upgradeDBPaths moves the databases from their locations prior to mass
...
func upgradeDBPaths() error {
	...
	oldDbRoot := filepath.Join(oldMassHomeDir(), "db")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "mass.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "mass_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "mass_regtest.db"), "regtest")
...
// upgradeDataPaths moves the application data from its location prior to mass
...
func upgradeDataPaths() error {
	...
	oldHomePath := oldMassHomeDir()
	newHomePath := config.MassHomeDir
	...
	if FileExists(oldHomePath) && !FileExists(newHomePath) {
		// Create the new path.
		logging.CPrint(logging.INFO, "migrating application home path", logging.LogFormat{"from": oldHomePath, "to": newHomePath})
		err := os.MkdirAll(newHomePath, 0700)
		if err != nil {
			return err
		}

		// Move old mass.conf into new location if needed.
		...
		if ohpEmpty {
			...
			str := fmt.Sprintf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to mass as new versions require it.
