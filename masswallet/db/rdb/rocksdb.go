...
func (b *rocksBucket) innerKeyForIterator(key []byte) []byte {
	kl := len(key)
	buf := make([]byte, b.pathLen+kl+1)
	copy(buf[:], []byte(b.path))
	copy(buf[b.pathLen:b.pathLen+1], []byte(bucketPathSep))
...
func (b *rocksBucket) NewIterator(slice *db.Range) db.Iterator {
	if slice == nil {
		slice = &db.Range{}
	}
...
func (it *rocksIterator) Key() []byte {
	...
	return append([]byte{}, k.Data()[it.b.pathLen+1:]...)
}
